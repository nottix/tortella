...
\item L’ingersso (\textit{join}\index{join}) e l’uscita (\textit{leave}\index{leave}) da una chat room esistente;
...
\item La comunicazione tra peer (basata su un opportuno protocollo da definire) deve avvenire usando come “protocollo di trasporto” il \textit{protocollo HTTP}\index{protocollo HTTP}.
